...
Це не просто популярність за кількістю переглядів. Популярність вимірює лише загальне число оцінок. Високий PageRank означає, що фільм є системоутворюючим хабом у графі. Його високо оцінили ті користувачі, які самі активно дивляться та високо оцінюють інші важливі фільми. Це показник авторитетності та центрального місця стрічки в мережі рекомендацій.
...
Відповіді на питання:
   ...
   1. Чи відповідають кластери реальним групам інтересів?

Так, алгоритм автоматично розділив користувачів на групи за смаками.

   2. Як це було перевірено?

Для кожного знайденого кластера (communityId) ми зробили вибірку високооцінених фільмів та просумували їхні жанри. Порівняння ТОП-3 жанрів між різними кластерами показало відмінності у вподобаннях глядачів.
...
Відповіді на питання:
   ...
   1. Наскільки тісний світ у датасеті?

Світ виявився дуже тісним. У графі схожості користувачів між більшістю довільно вибраних людей можна побудувати шлях усього за 1–3 кроки.

   2. Чи підтверджується гіпотеза 6 рукостискань?

Так, підтверджується з запасом. Середня довжина шляху між користувачами становить лише 2–3 кроки. Це пояснюється наявністю популярних фільмів та активних кіноманів, які виконують роль "містків" і роблять граф дуже щільно зв'язаним.
...
Найскладнішими (або практично неможливими) для ефективної реалізації в традиційному SQL є запити з пошуку найкоротшого шляху довільної довжини (Запит 6: shortestPath) та обходи багатьох хопів для рекомендаційних систем (Запит 5: Collaborative Filtering).

У реляційній БД пошук шляху невизначеної довжини вимагає використання рекурсивних спільних табличних виразів (WITH RECURSIVE). Рекурсивні JOIN-и по таблиці з мільйоном записів створюють величезне обчислювальне навантаження, оскільки реляційні БД змушені створювати тимчасові таблиці, перебирати індекси та контролювати відсутність циклів на кожному кроці рекурсії.

Приклад того, як складно виглядав би еквівалентний запит найкоротшого шляху в SQL:

```sql
WITH RECURSIVE PathFinder AS (
    SELECT user_id, movie_id, 1 AS depth, ARRAY[user_id] AS visited_users
    FROM Ratings
    WHERE user_id = 1

    UNION ALL

    SELECT r2.user_id, r2.movie_id, pf.depth + 1, pf.visited_users || r2.user_id
    FROM PathFinder pf
    JOIN Ratings r1 ON pf.movie_id = r1.movie_id
    JOIN Ratings r2 ON r1.movie_id = r2.movie_id
    WHERE NOT (r2.user_id = ANY(pf.visited_users)) AND pf.depth < 10
)
...
LIMIT 1;
```
У Cypher цей самий запит записується в один рядок:

```cypher
MATCH p = shortestPath((u1:User {userId: 1})-[:RATED*..10]-(u2:User {userId: 2})) RETURN p;
```

Різниця полягає в тому, що Neo4j використовує механізм Index-Free Adjacency (вузли містять прямі фізичні вказівники на сусідні ребра в пам'яті), тому граф переходить від вузла до вузла за час $O(1)$, тоді як SQL змушений робити повторні об'єднання таблицій (JOIN) та пошуки по індексах з часовою складністю $O(\log N)$


   3. Де граф програє? Для яких задач із цим датасетом реляційна модель підійшла б краще? Наприклад: агрегація по всіх користувачах, звіти, експорт даних.

   Графові бази даних поступаються реляційним (SQL) у задачах глобальної аналітики (OLAP), суцільної агрегації даних по всій системі та формування прямокутних табличних звітів. Якщо бізнес-задача вимагає розрахувати глобальні показники (наприклад, "порахувати середній рейтинг та сумарну кількість оцінок у розрізі всіх вікових груп і професій користувачів за останній рік"), SQL-база виконає це значно швидше. Реляційні системи оптимізовані для послідовного зчитування великих масивів даних із диска (особливо колонкові БД, такі як ClickHouse або PostgreSQL з відповідними індексами), тоді як графова база змушена сканувати всі вузли та обходити ребра окремо, що споживає значно більше оперативної пам'яті та ресурсів CPU. Крім того, реляційна модель набагато краще підходить для експорту даних у CSV/Excel, групових пакетних оновлень (UPDATE) та суворої валідації схеми через первинні й зовнішні ключі (Primary/Foreign Keys).
   
   3. Покращення схеми. Які зміни в схемі прискорили б конкретні запити з частини 3? Розгляньте хоча б два запити.

1) Частина 3, запит 1 (Фільми Thriller із рейтингом > 4.0):

Проблема: Запит змушений динамічно обходити всі ребра :RATED для кожного фільму жанру та обчислювати avg(r.rating) на льоту.

Оптимізація: Додати денормалізовані властивості безпосередньо у вузол Movie: m.avgRating та m.ratingsCount (які оновлюються фоновим процесом або тригером при додаванні оцінки). Тоді запит замість динамічного підрахунку тисяч ребер перетвориться на швидкий пошук за індексом по вузлах: MATCH (m:Movie)-[:IN_GENRE]->(:Genre {name: 'Thriller'}) WHERE m.avgRating > 4.0 RETURN m.

2) Частина 3, запит 1 (Користувачі з понад 50 п'ятірками):

Проблема: База змушена зчитувати властивість {rating: 5} з кожного ребра :RATED, що вимагає додаткових операцій введення-виведення (I/O) для кожного зв'язку.

Оптимізація: Використовувати окремий тип ребра для найвищих оцінок, наприклад (User)-[:RATED_FIVE]->(Movie). У Neo4j різні типи ребер зберігаються в окремих списках суміжності. Запит MATCH (u:User)-[r:RATED_FIVE]->(:Movie) буде відбирати лише потрібні зв'язки без необхідності зчитувати та фільтрувати властивості всередині ребер.
